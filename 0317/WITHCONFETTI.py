        ...
        if self.selected_profile_data:
            ...
            for key in important_keys:
                if key in self.selected_profile_data:
                    ...
                    self.game.screen.blit(data_text, (data_panel.x + 10, y_offset))
                    y_offset += 30
            
            # Then display other keys
            for key, value in self.selected_profile_data.items():
                ...
                # Check if we still have room in the panel
                if y_offset <= data_panel.y + 310:
                    self.game.screen.blit(data_text, (data_panel.x + 10, y_offset))
                    y_offset += 30
                else:
                    more_text = self.small_font.render("... more data not shown", True, pygame.Color('yellow'))
                    self.game.screen.blit(more_text, (data_panel.x + 10, y_offset))
                    break
