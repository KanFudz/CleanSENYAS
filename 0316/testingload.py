...
class LoadGameState(State):
    def __init__(self, game, video_key, audio_file=None):
        ...
        self.scroll_offset = 0  # Initialize scroll offset for profile list
        self.data_scroll_offset = 0  # Initialize scroll offset for profile data
        self.dragging = False  # Flag to indicate if dragging is in progress
        self.drag_start_y = 0  # Starting y position of the drag
        self.data_dragging = False  # Flag for data panel dragging
        self.data_drag_start_y = 0  # Starting y position for data panel drag
    ...
    def load_profile_data(self, profile_name):
        ...
        try:
            with open(f"saves/{profile_name}.json", "r") as f:
                ...
                self.data_scroll_offset = 0  # Reset scroll offset when loading new profile
    ...
    def handle_event(self, event):
        if event.type == pygame.MOUSEMOTION:
            if self.dragging:
                ...
                # Calculate the difference in y position for data panel
                delta_y = event.pos[1] - self.data_drag_start_y
                self.data_scroll_offset += delta_y
                
                # Calculate max scroll based on content height
                if self.selected_profile_data:
                    max_data_height = (len(self.selected_profile_data) + 1) * 30  # +1 for the header
                    visible_height = 310  # Visible height in the panel
                    max_scroll = max(0, max_data_height - visible_height)
                    self.data_scroll_offset = max(min(self.data_scroll_offset, 0), -max_scroll)
                
                self.data_drag_start_y = event.pos[1]
            else:
                # Check if mouse is over data panel
                data_panel = pygame.Rect(50, 150, 280, 330)
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 1:  # Left mouse button
                # Check if mouse is over data panel
                data_panel = pygame.Rect(50, 150, 280, 330)
                if data_panel.collidepoint(event.pos) and self.selected_profile_data:
            ...
            for profile_name, button_rect in self.profile_buttons:
                if button_rect.move(0, self.scroll_offset).collidepoint(event.pos):
                    ...
                    self.load_profile_data(profile_name)  # Load the profile data
                    return
            ...
            if self.delete_button_collision.collidepoint(event.pos) and self.selected_profile:
                ...
                self.selected_profile_data = None  # Clear the profile data
        ...
        if event.type == pygame.MOUSEWHEEL:
            ...
            if data_panel.collidepoint(mouse_pos) and self.selected_profile_data:
                # Scroll data panel
                self.data_scroll_offset += event.y * 20  # Adjust scroll speed
                
                # Calculate max scroll based on content height
                max_data_height = (len(self.selected_profile_data) + 1) * 30  # +1 for the header
                visible_height = 310  # Visible height in data panel
                max_scroll = max(0, max_data_height - visible_height)
                self.data_scroll_offset = max(min(self.data_scroll_offset, 0), -max_scroll)
            else:
                # Scroll profile list
    ...
    def render(self):
        ...
        # Draw the selected profile data on the left side with scrolling
        if self.selected_profile_data:
            # Create a panel for the save data
            data_panel = pygame.Rect(50, 150, 280, 330)
            pygame.draw.rect(self.game.screen, (30, 30, 60), data_panel)
            pygame.draw.rect(self.game.screen, (0, 200, 200), data_panel, 2)
            
            # Create a clipping rect for the data panel (excluding the header)
            data_clip_rect = pygame.Rect(data_panel.x, data_panel.y + 50, data_panel.width, data_panel.height - 50)
            
            # Draw the header (fixed at the top)
            header_text = self.font.render(f"Profile: {self.selected_profile}", True, pygame.Color('white'))
            header_rect = header_text.get_rect(x=data_panel.x + 10, y=data_panel.y + 10)
            self.game.screen.blit(header_text, header_rect)
            
            # Indicate scrollable content
            if len(self.selected_profile_data) * 30 > data_panel.height - 50:
                scroll_hint = self.small_font.render("(Scroll to see more)", True, pygame.Color('yellow'))
                self.game.screen.blit(scroll_hint, (data_panel.x + 10, data_panel.y + 40))
            
            # Create a surface for the scrollable data
            max_data_height = len(self.selected_profile_data) * 30 + 10  # Height based on number of items + padding
            data_surface = pygame.Surface((data_panel.width - 4, max_data_height), pygame.SRCALPHA)
            data_surface.fill((30, 30, 60, 0))  # Transparent background
            
            # List of important keys to display first if they exist
            important_keys = ["name", "progress", "location", "level", "coins", "health"]
            ...
            for key in important_keys:
                if key in self.selected_profile_data:
                    data_items.append((key, self.selected_profile_data[key]))
            
            # Then add other keys
            for key, value in self.selected_profile_data.items():
                if key not in important_keys:
                    data_items.append((key, value))
            
            # Draw data items onto the data surface
            y_offset = 10
            for key, value in data_items:
                ...
                data_text = self.small_font.render(f"{key}: {value_str}", True, pygame.Color('white'))
                data_surface.blit(data_text, (10, y_offset))
                y_offset += 30
            
            # Draw scrollable data surface onto screen with clipping
            orig_clip = self.game.screen.get_clip()
            self.game.screen.set_clip(data_clip_rect)
            self.game.screen.blit(data_surface, (data_panel.x + 2, data_panel.y + 52 + self.data_scroll_offset))
            self.game.screen.set_clip(orig_clip)
        
        # Draw profile selection buttons with scrolling
        for profile_name, button_rect in self.profile_buttons:
            adjusted_rect = button_rect.move(0, self.scroll_offset)
            pygame.draw.rect(self.game.screen, (50, 50, 150), adjusted_rect)
            pygame.draw.rect(self.game.screen, (0, 200, 200), adjusted_rect, 2)
            
            profile_text = self.font.render(profile_name, True, pygame.Color('white'))
            text_rect = profile_text.get_rect(center=adjusted_rect.center)
            self.game.screen.blit(profile_text, text_rect)
            
            # Highlight the selected profile
            if self.selected_profile == profile_name:
                pygame.draw.rect(self.game.screen, (255, 255, 0), adjusted_rect, 3)
        
        # Draw buttons (BACK, LOAD, DELETE)
        self.game.screen.blit(self.back_button_img, (30, 500))
        self.game.screen.blit(self.load_button_img, (400, 500))
        self.game.screen.blit(self.delete_button_img, (750, 500))
        
        # Highlight hovered button
        if self.hovered_button:
            pygame.draw.rect(self.game.screen, (255, 255, 0), self.hovered_button, 3)
