...
class LoadGameState(State):
    def __init__(self, game, video_key, audio_file=None):
        ...
        self.data_scroll_offset = 0  # Initialize data scroll offset
        self.data_dragging = False  # Flag to indicate if data dragging is in progress
        self.data_drag_start_y = 0  # Starting y position of the data drag
    ...
    def handle_event(self, event):
        ...
        # Handle scrolling for the save data panel
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 1:  # Left mouse button
                if hasattr(self, 'data_panel') and self.data_panel.collidepoint(event.pos):
        ...
        if event.type == pygame.MOUSEMOTION:
            if self.data_dragging:
                ...
                if abs(delta_y) > 5:  # Ignore small movements below the threshold
                    max_scroll_speed = 5  # Limit the maximum scroll speed
                    delta_y = max(-max_scroll_speed, min(max_scroll_speed, delta_y))  # Clamp delta_y
                    deceleration_factor = 0.5  # Apply deceleration
                    self.data_scroll_offset += (delta_y * deceleration_factor)
                    self.data_scroll_offset = int(self.data_scroll_offset)  # Ensure it's an integer
                    self.data_scroll_offset = max(min(self.data_scroll_offset, 0), -max(0, self.y_offset - self.data_panel.y - 50))  # Limit scrolling
                    self.data_drag_start_y = event.pos[1]  # Update the starting y position for the next motion event
    ...
    def render(self):
        ...
        if self.selected_profile_data:
            ...
            header_text = self.small_font.render(f"Profile: {self.selected_profile}", True, pygame.Color('white'))
            ...
            created_at_text = self.small_font.render(f"Created at: {self.selected_profile_data.get('created at', 'N/A')}", True, pygame.Color('white'))
            ...
            # Draw the progress
            self.y_offset = self.data_panel.y + 70 + self.data_scroll_offset
            ...
            self.game.screen.blit(progress_text, (self.data_panel.x + 10, self.y_offset))
            self.y_offset += 30
            
            lines_to_draw = []
            for category, lessons in progress.items():
                category_text = f"{category.replace('_', ' ').title()}:"
                lines_to_draw.append(category_text)
                ...
                for lesson_type, lesson_values in grouped_lessons.items():
                    ...
                    lines_to_draw.append(f"{lesson_type}:")
                    lines_to_draw.extend(formatted_lines)
            
            # Limit the number of visible lines to 8
            visible_lines = lines_to_draw[int(self.data_scroll_offset):int(self.data_scroll_offset) + 8]
            
            for line in visible_lines:
                lesson_text = self.small_font.render(line, True, pygame.Color('white'))
                self.game.screen.blit(lesson_text, (self.data_panel.x + 20, self.y_offset))
                self.y_offset += 30
...
class StarQuestState(State):
    ...
    def update(self):
        ...
        if self.celebration_active:
            ...
            if self.celebration_start_time and time.time() - self.celebration_start_time > 2:
                self.celebration_active = False
                self.celebration_start_time = None
                self.confetti_particles = []
