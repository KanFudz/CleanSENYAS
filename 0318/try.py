...
class LoadGameState(State):
    ...
    def handle_event(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 1:  # Left mouse button
                if self.data_panel.collidepoint(event.pos):  # Check if the click is within the data panel
        ...
        if event.type == pygame.MOUSEMOTION:
            if self.data_dragging:
                ...
                self.data_scroll_offset = max(
                    min(self.data_scroll_offset, 0),
                    -max(0, (len(self.lines_to_draw) * 30) - self.data_panel.height)
                )
    ...
    def render(self):
        ...
        if self.selected_profile_data:
            ...
            self.lines_to_draw = []  # Store lines to draw as an instance variable
            ...
            visible_lines = self.lines_to_draw[int(self.data_scroll_offset):int(self.data_scroll_offset) + 8]
