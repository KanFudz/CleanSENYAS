    ...
    def render(self):
        ...
        if self.selected_profile_data:
            ...
            for line in wrapped_name:
                name_text = self.small_font.render(line, True, pygame.Color('green'))
                name_rect = name_text.get_rect(x=self.data_panel.x + 10, y=y_offset)
                self.game.screen.blit(name_text, name_rect)
                y_offset += 25

            # Draw creation date
            created_at_text = self.small_font.render(
                f"Created at: {self.selected_profile_data.get('created at', 'N/A')}", 
                True, pygame.Color('green')
            )
            created_at_rect = created_at_text.get_rect(x=self.data_panel.x + 10, y=y_offset)
            ...
            progress_rect = progress_text.get_rect(x=self.data_panel.x + 10, y=y_offset + 25)
            self.game.screen.blit(progress_text, progress_rect)
            
            # Draw scrollable progress content
            content_y = y_offset + 55
            line_height = 30
            
            visible_area = pygame.Rect(
                self.data_panel.x,
                self.data_panel.y + 100,
                self.data_panel.width,
                self.data_panel.height - 100
            )
            
            for i, line in enumerate(self.lines_to_draw):
                y_pos = content_y + (i * line_height) + self.data_scroll_offset
                
                if y_pos < visible_area.top or y_pos > visible_area.bottom:
                    continue
                
                indent = 10 if ":" in line else 30
                if line.startswith("  "):
                    indent = 30
                elif any(word in line for word in ["Alphabets", "Number", "Phrase", "Fingerspelling"]):
                    indent = 20
                ...
                if result.multi_hand_landmarks and not self.confetti_triggered:
                    for hand_landmarks in result.multi_hand_landmarks:
                        ...
                        if input_data.shape[1] == self.input_details[0]['shape'][1]:
                            ...
                            if confidence >= 0.99 and self.labels[prediction] == self.expected_letter:
                                self.correct = True
                ...
                if result.multi_hand_landmarks and not self.confetti_triggered:
                    for hand_landmarks in result.multi_hand_landmarks:
                        ...
                        if input_data.shape[1] == self.input_details[0]['shape'][1]:
                            ...
                            # Adjust confidence threshold to 0.99
                            confidence = output_data[0][prediction]
                            if confidence >= 0.99 and self.labels[prediction] == str(self.expected_number):
                                self.correct = True
                                if self.start_time is None:
                                    self.start_time = time.time()  # Start the timer
                                    self.save_progress()  # Save progress when correct
                            ...
                            try:
                                ...
                                if max_prob > self.threshold:
                                    ...
                                    if (len(self.predictions) >= self.min_consecutive_predictions and 
                                        ...
                                        if current_pred.upper() == self.expected_phrase:
                                            self.correct = True
                                            if not self.confetti_triggered:
                                                self.confetti_particles = [Confetti(1024, 600) for _ in range(100)]
                                                self.confetti_triggered = True
