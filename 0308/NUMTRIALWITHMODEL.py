...
class NumberDisplayState(State):
    def __init__(self, game, image_path, webcam_position=(700, 150), webcam_size=(300, 225)):
        ...
        # Load ASL Model
        model_path = os.path.join(os.getcwd(), "Numbers", "asl_number_classifier.tflite")
        if not os.path.exists(model_path):
            raise FileNotFoundError(f"Error: Model file '{model_path}' not found. Check the file path.")
    ...
    def get_scaled_dimensions(self, image, max_width, max_height):
        """ Scales the image while maintaining aspect ratio within max dimensions """
        ...
        if ret:
            ...
            if result.multi_hand_landmarks:
                for hand_landmarks in result.multi_hand_landmarks:
                    ...
                    self.prediction = np.argmax(output_data)
            ...
            pygame.draw.rect(self.game.screen, (255, 255, 255), pygame.Rect(self.webcam_position, self.webcam_size), 5)
            
            if self.prediction is not None:
                font = pygame.font.Font(None, 50)
                text_surface = font.render(f"Prediction: {self.labels[self.prediction]}", True, (0, 255, 0))
    ...
    def handle_event(self, event):
        if event.type == pygame.MOUSEBUTTONDOWN:
            if self.back_button_rect.collidepoint(event.pos):
                self.game.change_state("playing_numbers")
