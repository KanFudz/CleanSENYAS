...
class PhraseDisplayState(State):
    def __init__(self, game, image_path, expected_phrase, webcam_position=(700, 150), webcam_size=(300, 225)):
        ...
        # Load the MLP model
        self.model = self.load_mlp_model("MODEL/mlp_model.pkl")
    ...
    def load_mlp_model(self, model_path):
        """Load the MLP model from a pickle file."""
        import pickle
    ...
    def update(self):
        if not isinstance(self.webcam_position, tuple) or len(self.webcam_position) != 2 or not all(isinstance(i, int) for i in self.webcam_position):
            self.webcam_position = (700, 150)  # Fallback to default position
            print("Warning: Invalid webcam_position detected. Resetting to default (700, 150).")
        ...
        if self.webcam:
            ...
            if ret:
                ...
                if self.correct is True:
                    ...
                    result_text = None  # No gesture detected, display nothing

                if result_text:  # Only render text if it's not None
                    result_surface = self.game.font.render(result_text, True, pygame.Color('white'))
                ...
                # Convert the webcam frame to a surface and blit it
                webcam_surface = pygame.surfarray.make_surface(cv2.resize(cv2.cvtColor(webcam_frame, cv2.COLOR_BGR2RGB), self.webcam_size).swapaxes(0, 1))
                if isinstance(self.webcam_position, tuple) and len(self.webcam_position) == 2:
                    self.game.screen.blit(webcam_surface, self.webcam_position)
                else:
                    raise ValueError(f"Invalid webcam_position: {self.webcam_position}")
