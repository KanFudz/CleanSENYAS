...
class AlphabetDisplayState(State):
    ...
    def exit(self):
        ...
        self.hands.close()  # Close the Mediapipe Hands instance
        
class GalaxyExplorerNumberState(VideoState):
    def __init__(self, game, video_key, next_state=None):
        super().__init__(game, video_key, next_state)  # Remove `audio_file`
...
class CosmicCopyState(State):
    ...
    def enter(self):
        ...
        if selected_category == "alphabet":
            ...
            model_path = os.path.join("MODEL", "asl_mlp_model.tflite")  # Load Alphabet Model
            ...
            model_path = os.path.join("MODEL", "asl_number_classifier.tflite")  # Load Number Model
            ...
            model_path = os.path.join("MODEL", "asl_phrase_model.tflite")  # Load Phrase Model
            expected_output = phrase

        # Debugging: Print the chosen category and model
        print(f"Category: {selected_category}, Model: {model_path}, Expected: {expected_output}")

        # Transition to Cosmic Display State with the correct model
...
class CosmicDisplayState(State):
    def __init__(self, game, image_path, model_path, expected_output):
        ...
        # Debugging: Print the loaded model
        ...
        # Load the correct labels based on the model type
        if "mlp_model" in model_path:
            self.labels = [chr(i) for i in range(ord('A'), ord('Z') + 1)]  # Alphabet Model
        elif "number_classifier" in model_path:
            self.labels = [str(i) for i in range(10)]  # Number Model
        else:
            self.labels = ["goodbye", "hello", "iloveyou", "no", "please", "sorry", "thankyou", "yes"]  # Phrase Model
    ...
    def update(self):
        ...
        if ret:
            ...
            # Display Webcam
            webcam_surface = pygame.surfarray.make_surface(cv2.resize(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB), self.webcam_size).swapaxes(0, 1))
            self.game.screen.blit(webcam_surface, self.webcam_position)
